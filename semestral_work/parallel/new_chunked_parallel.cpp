...
void naive(const uint8_t* text, size_t strLen, const string* patterns, size_t pattCnt, int threads, int chunkSize) {
	...
	#pragma omp parallel default(shared) private(chunk)
	{
		...
		for(chunk = 0; chunk < strLen; chunk += chunkSize) {
			...
			for(size_t patt = 0; patt < pattCnt; patt++) {
				...
				size_t chunkEnd = min((size_t)chunkSize, strLen - chunk);
...
void kmp(const uint8_t* text, size_t strLen, const string* patterns, size_t pattCnt, int* shifts, int threads, int chunkSize) {
