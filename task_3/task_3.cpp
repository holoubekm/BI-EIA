...
const long pages_cnt = 16384; //numbers
const long pages_cnt_bits = 14;
const long pages_cnt_bits_rest = 32 - pages_cnt_bits;
const long page_size = 67108864 / pages_cnt; //bits
const long page_size_bits = pages_cnt_bits + 6;

// const long page_size = 4294967296 / pages_cnt; //bits
// const long page_size_bits = 32 - pages_cnt_bits - 5;
...
public:
  MySet() {
    length = 0;
    pages = new uint64_t*[pages_cnt];
    sizes = new uint32_t*[pages_cnt];
    sums = new uint32_t*[pages_cnt];
  ...
  int32_t inter1(int32_t valA, int32_t valB) {
    ...
    if(aPageInx == bPageInx) {
      uint64_t* page = pages[aPageInx];
      uint32_t* sum = sums[aPageInx];
      found += __builtin_popcountll((uint64_t)(page[aBucketInx] >> aOffset));
      found += __builtin_popcountll((uint64_t)(page[bBucketInx] << (0x3F - bOffset)));
      found += sum[bBucketInx - 1] - sum[aBucketInx];
    } else {
      uint64_t* page = pages[aPageInx];
      uint32_t* sum = sums[aPageInx];
      if(page != nullptr) {
        found += __builtin_popcountll((uint64_t)(page[aBucketInx] >> aOffset));
        if(aBucketInx != 0)
          found += sum[page_size - 1] - sum[aBucketInx];
      }
      page = pages[bPageInx];
      sum = sums[bPageInx];
      if(page != nullptr) {
    ...
    if(changed) {
      for(int x = 0; x < pages_cnt; x++) {
        if(sizes[x] == nullptr)
          continue;
        uint32_t*& sum = sums[x];
        uint32_t*& size = sizes[x];
        sum[0] = size[0];
        for(int y = 1; y < page_size; y++)
          sum[y] = sum[y - 1] + size[y];
      } changed = false;
    }
  ...
  int insert(int32_t *values, int n) {
    ...
    for(int x = 0; x < n; x++) {
      ...
      offsetVal = 1ULL << (val & 0x3F);
      uint64_t*& page = pages[pageInx];
      uint32_t*& size = sizes[pageInx];
      uint32_t*& sum = sums[pageInx];
      uint32_t& count = counts[pageInx];
      if(page == nullptr) {
        page = new uint64_t[page_size];
        size = new uint32_t[page_size];
        sum = new uint32_t[page_size];
        for(int y = 0; y < page_size; y++)
          size[y] = page[y] = 0;
      }

      uint64_t& bucket = page[bucketInx];
      if(!(bucket & offsetVal)) {
        bucket |= offsetVal;
  ...
  MySet *Union(MySet *A, MySet *B) {
    for(int x = 0; x < pages_cnt; x++) {
      ...
      if(A->pages[x] != nullptr || B->pages[x] != nullptr) {
        ...
        sum = new uint32_t[page_size];
        for(int y = 0; y < page_size; y++) {
          uint64_t& bucket = page[y];
          bucket = 0;
          if(A->pages[x] != nullptr)
            bucket |= A->pages[x][y];
          if(B->pages[x] != nullptr)
            bucket |= B->pages[x][y];
          uint32_t cnt = __builtin_popcountll(bucket);
          size[y] = cnt;
          length += cnt;
          count += cnt;
        }
